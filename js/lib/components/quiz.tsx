...
class AnswerStorage {
  ...
  save(answers: TaggedAnswer[], confirmedDone: boolean, attempt: number, wrongAnswers?: number[]) {
  ...
  return (
    <div className="exit-explanation">
      ...
      <div style={{ display: expanded ? "block" : "none" }}>
        We want to know how much you are learning that can be recalled without assistance. Please
        complete the quiz without re-reading the text, e.g. by opening it in another tab.
      </div>
    </div>
  );
};
  ...
  ({ quiz, name, fullscreen, cacheAnswers, allowRetry, onFinish }) => {
    ...
    let onSubmit = action((answer: TaggedAnswer) => {
      ...
      if (state.attempt == 0) {
        ...
        if (wrongAnswerIdx == state.wrongAnswers!.length - 1) state.index = quiz.questions.length;
    ...
    let wrapperClass = classNames("mdbook-quiz-wrapper", { expanded: showFullscreen });
    let onExit = action(() => {
      state.started = false;
      state.index = 0;
      state.answers = [];
